    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                // 使用條件編譯提供適合不同iOS版本的導航方式
                #if os(iOS) && compiler(>=5.5) && canImport(SwiftUI) && !(arch(i386) || arch(x86_64))
            ...
            if #available(iOS 16.0, *) {
                // 對於iOS 16+，使用新的modifier
            } else {
                // 對於iOS 16之前的版本，使用舊的navigationViewStyle
...
struct p40_Previews: PreviewProvider {
